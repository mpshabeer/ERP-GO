...
public class SalesInvoiceDocument : IDocument
{
    ...
    public SalesInvoiceDocument(SalesInvoice invoice, AppSettingsModel settings)
    {
        _invoice = invoice;
        _settings = settings;
    }
    ...
    void ComposeContent(IContainer container)
    {
        container.Column(col =>
        {
            ...
            col.Item().PaddingTop(12).AlignRight().Width(230).Table(t =>
            {
                ...
                TR("Sub Total:", $"{_invoice.SubTotal:N2}");
                if (_invoice.DiscountAmount > 0)
                    TR($"Discount ({_invoice.DiscountPercent:N1}%):", $"- {_invoice.DiscountAmount:N2}");
                if (_invoice.TaxAmount > 0)
                    TR($"Tax ({_invoice.TaxPercent:N1}%):", $"+ {_invoice.TaxAmount:N2}");
