...
public class SalesItemReportDocument : IDocument
{
    private readonly SalesReportRequest _request;
    ...
    private const string PrimaryColor = "#1565C0"; // Blue for Sales
    private const string AccentColorHex = "#E3F2FD"; // Light Blue
    private const string TextLight = "#78909C";
    private const string LineColorHex = "#E0E0E0";

    public SalesItemReportDocument(SalesReportRequest request, List<ItemWiseReportLine> lines, AppSettingsModel settings)
    {
        _request = request;
        _lines = lines;
        _settings = settings;
    ...
    void ComposeHeader(IContainer container)
    {
        container.Column(col =>
        {
            col.Item().Row(row =>
            {
                ...
                {
                    c.Item().Text("ITEM-WISE SALES REPORT").FontSize(16).Bold().FontColor(PrimaryColor);
        ...
        container.Column(col =>
        {
            col.Item().PaddingTop(12).Table(table =>
            {
                table.ColumnsDefinition(cd =>
                {
                    ...
                    cd.ConstantColumn(70);         // Inv No
                    cd.RelativeColumn(1.5f);       // Customer
                ...
                table.Header(h =>
                {
                    ...
                    h.Cell().Element(HC).Text("Inv No").FontSize(8).Bold().FontColor(Colors.White);
                    h.Cell().Element(HC).Text("Customer").FontSize(8).Bold().FontColor(Colors.White);
                ...
                if (_lines.Any())
                {
                    table.Cell().ColumnSpan(10).Background(AccentColorHex).BorderTop(1).BorderColor(PrimaryColor).PaddingVertical(6).PaddingHorizontal(4).AlignRight().Text("TOTAL SALES:").FontSize(9).Bold().FontColor(PrimaryColor);
            ...
            if (!_lines.Any())
            {
                col.Item().PaddingTop(20).AlignCenter().Text("No sales transactions found for the selected criteria.").FontSize(10).Italic().FontColor(TextLight);
            }
        });
    }
