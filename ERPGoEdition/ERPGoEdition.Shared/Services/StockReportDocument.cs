...
{
    private readonly StockReportRequest _request;
    private readonly List<StockReportLine> _lines;
    ...
    private const string PrimaryColor = "#2E7D32"; // Dark Green for Reporting
    private const string AccentColorHex = "#E8F5E9"; // Light Green
    private const string TextLight = "#78909C";
    private const string LineColorHex = "#E0E0E0";

    public StockReportDocument(StockReportRequest request, List<StockReportLine> lines, AppSettingsModel settings)
    {
        _request = request;
        _lines = lines;
        _settings = settings;
    ...
    void ComposeHeader(IContainer container)
    {
        container.Column(col =>
        {
            col.Item().Row(row =>
            {
                ...
                {
                    c.Item().Text("STOCK REGISTER REPORT").FontSize(16).Bold().FontColor(PrimaryColor);
        ...
        container.Column(col =>
        {
            col.Item().PaddingTop(12).Table(table =>
            {
                table.ColumnsDefinition(cd =>
                {
                    ...
                    cd.ConstantColumn(70);         // Item Code
                    cd.RelativeColumn(2.5f);       // Item Name
                    cd.RelativeColumn(1.2f);       // Category
                    cd.ConstantColumn(40);         // Unit
                    cd.RelativeColumn(1);          // Opening
                    cd.RelativeColumn(1);          // Inward
                    cd.RelativeColumn(1);          // Outward
                    cd.RelativeColumn(1);          // Closing
                    cd.RelativeColumn(1);          // Rate
                    cd.RelativeColumn(1.2f);       // Value
                });
                ...
                table.Header(h =>
                {
                    ...
                    h.Cell().Element(HC).Text("Code").FontSize(8).Bold().FontColor(Colors.White);
                    ...
                    h.Cell().Element(HC).AlignRight().Text("Opening").FontSize(8).Bold().FontColor(Colors.White);
                    h.Cell().Element(HC).AlignRight().Text("Inward").FontSize(8).Bold().FontColor(Colors.White);
                    h.Cell().Element(HC).AlignRight().Text("Outward").FontSize(8).Bold().FontColor(Colors.White);
                    h.Cell().Element(HC).AlignRight().Text("Closing").FontSize(8).Bold().FontColor(Colors.White);
                    h.Cell().Element(HC).AlignRight().Text("Rate").FontSize(8).Bold().FontColor(Colors.White);
                    h.Cell().Element(HC).AlignRight().Text("Stock Value").FontSize(8).Bold().FontColor(Colors.White);
                });
                ...
                for (int i = 0; i < _lines.Count; i++)
                {
                    ...
                    table.Cell().Element(DC).Text(item.ItemCode).FontSize(8).SemiBold();
                    ...
                    table.Cell().Element(DC).AlignRight().Text(item.OpeningQty.ToString("N2")).FontSize(8);
                    table.Cell().Element(DC).AlignRight().Text(item.InwardQty.ToString("N2")).FontSize(8).FontColor(Colors.Green.Darken2);
                    table.Cell().Element(DC).AlignRight().Text(item.OutwardQty.ToString("N2")).FontSize(8).FontColor(Colors.Red.Darken2);
                    table.Cell().Element(DC).AlignRight().Text(item.ClosingQty.ToString("N2")).FontSize(8).Bold();
                    table.Cell().Element(DC).AlignRight().Text(item.ItemRate.ToString("N2")).FontSize(8).FontColor(TextLight);
                    table.Cell().Element(DC).AlignRight().Text(item.StockValue.ToString("N2")).FontSize(8).Bold().FontColor(PrimaryColor);
                }

                if (_lines.Any())
                {
                    table.Cell().ColumnSpan(10).Background(AccentColorHex).BorderTop(1).BorderColor(PrimaryColor).PaddingVertical(6).PaddingHorizontal(4).AlignRight().Text("TOTAL STOCK VALUE:").FontSize(9).Bold().FontColor(PrimaryColor);
                    table.Cell().Background(AccentColorHex).BorderTop(1).BorderColor(PrimaryColor).PaddingVertical(6).PaddingHorizontal(4).AlignRight().Text(_lines.Sum(x => x.StockValue).ToString("N2")).FontSize(9).Bold().FontColor(PrimaryColor);
                }
            });

            if (!_lines.Any())
            {
                col.Item().PaddingTop(20).AlignCenter().Text("No stock transactions found for the selected criteria.").FontSize(10).Italic().FontColor(TextLight);
            }
        });
    }
